...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor], prompt_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for idx, (x, masks) in enumerate(zip(all_x, masks_list)):
            ...
            elif prompt_list[0] != None:
                n_prompts = prompt_list[0].shape[1]
                x_norm = self.norm(x)
                x_norm_cls_reg = x_norm[:, : self.n_storage_tokens + 1]
                x_norm_prompts = x_norm[:, self.n_storage_tokens + 1 : n_prompts + 1]
                x_norm_patch = x_norm[:, self.n_storage_tokens + n_prompts + 1 :]

                output.append(
                    {
                        "x_norm_clstoken": x_norm_cls_reg[:, 0],
                        "x_storage_tokens": x_norm_cls_reg[:, 1:],
                        "x_norm_prompts": x_norm_prompts,
